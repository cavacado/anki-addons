    ...
    for dictionary in dictionaries:
        if expression in dictionary:
            ...
            for j in index:
                ...
                examples.append("<div id='eng_test'>%s<br><span id='eng_sentence'>%s</span></div>" % tuple(example.split('\t')))
        else:
            ...
            if match:
                if match.group(1).strip():
                    ...
                    maxitems -= len(res)
                    examples.extend(res)
    # program doesnt return any examples if it cant find any
    return examples
